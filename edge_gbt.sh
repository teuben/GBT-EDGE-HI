...
#--HELP
#
#  edge_gbt.sh   simulate an EDGE GBT spectrum.   See also hi_profile.py
#
...
plot=profile,rotcur,density  # show which plots can be made               #> CHECK rotcur,profile,density
...
if [[ "$plot" == *"profile"* ]]; then    
    tabplot $run.spec line=1,1  ycoord=0 headline="Spectrum around VLSR=$vlsr" yapp=3/xs
fi
